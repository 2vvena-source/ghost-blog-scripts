/*!
 * CATEGORY-LANDING v7 (external hosted)
 *
 * v7 변경 (v6 대비) — 리스트뷰만 수정:
 *   [!] 상하 굵은 테두리 제거 (사용자 지시)
 *   [!] no-image 카드 = 완전히 다른 레이아웃:
 *       좌측 빗금 자리(이미지 자리) 그대로 + 우측 = 알약태그 + 큰 제목 + 젤경 + 날짜
 *       (2행 구조, 라벨 없음 — with-image 와 다름)
 *   [!] with-image 카드 이미지 표시 버그 수정 — 부모 .gh-card-image 에 min-height 보장
 *
 * 배포: 2026-08-21
...
(function(){
  ...
  var CSS = ''
    ...
    + '  left: 0;'
    + '}'
    // 이미지 없는 카드: 대각선 사선 스트라이프 (티스토리 톤)
    + '.cl-feed.cl-view-list .gh-card.no-image {'
    ...
    + '  background-repeat: repeat;'
    + '  border-right: 1px solid var(--color, #0F3A3A);'
    + '}'
    + '.cl-feed.cl-view-list .gh-card.no-image .gh-card-image img {'
    + '  display: none !important;'
    + '}'
    // no-image 전용 레이아웃: 2행 구조, 라벨 없음
    + '.cl-feed.cl-view-list .gh-card.no-image .gh-card-wrapper {'
    + '  padding: 0 !important;'
    + '  display: flex !important;'
    + '  flex-direction: column !important;'
    + '}'
    // 태그 + 제목 = 상단 큰 블록 (라벨 ::before 숨김, 태그란/요약란 없음)
    + '.cl-feed.cl-view-list .gh-card.no-image .gh-card-title {'
    ...
    + '  font-family: "Cafe24Danjunghae","Gowun Batang","Nanum Myeongjo",serif !important;'
    + '}'
    + '.cl-feed.cl-view-list .gh-card.no-image .gh-card-title::before {'
    + '  display: none !important;'
    + '}'
    // 요약행 안 보이게
    + '.cl-feed.cl-view-list .gh-card.no-image .gh-card-excerpt {'
    + '  display: none !important;'
    + '}'
    // 날짜 행
    + '.cl-feed.cl-view-list .gh-card.no-image .gh-card-meta {'
    + '  padding: 0.5em 1.4em !important;'
    + '  min-height: auto !important;'
    + '  flex: 0 0 auto;'
    + '}'
    + '.cl-feed.cl-view-list .gh-card.no-image .gh-card-meta::before {'
    + '  display: none !important;'
    + '}'
    // ▷ 우측 정보영역 — 3행 구조
    ...
    + '}'
    // 개별 행 스타일
    // 상단 행: 알약 태그 + 제목 (태그는 JS 없이 CSS 만으로 제목 앞에 둘 수 없으므로
    //   .gh-card-tag 를 제목란 내부의 flex 아이템으로 배치)
    + '.cl-feed.cl-view-list .gh-card-title {'
    + '  font-size: 1em !important;'
    + '  gap: 0.8em;'
    + '  flex-wrap: wrap;'
    + '}'
    // 제목 실제 텍스트 — h3 안에 있을 수 있으므로 명시적 네이밍 불가
    //   .gh-card-title 자체를 flex row 로 쓰고, 그 안의 제목 텍스트에 명조 폰트 적용
  ...
  function injectCSS(){
    try {
      ...
      style.setAttribute('data-ddl-cat-landing', 'v7');
  ...
  // v6: 카드뷰 넘버링 삽입 (시리즈 sg-num 감성)
  //     리스트뷰 알약 태그 삽입 (제목란 안에 flex 아이템으로)
  function enrichCards(){
    try {
      var cards = document.querySelectorAll('.cl-feed > .gh-card, .cl-feed > article.gh-card');
      cards.forEach(function(card, idx){
        // 넘버링 (카드뷰 전용, 리스트뷰에서는 CSS 로 숨김)
        if (!card.querySelector('.cl-card-num')) {
          var wrap = card.querySelector('.gh-card-wrapper');
          if (wrap) {
            var num = document.createElement('div');
            num.className = 'cl-card-num';
            num.textContent = String(idx + 1).padStart(2, '0') + '.';
            wrap.insertBefore(num, wrap.firstChild);
          }
        }
        // 리스트뷰 알약 태그 (제목란 안 왼쪽에 배치)
        if (!card.querySelector('.cl-list-tag')) {
          var title = card.querySelector('.gh-card-title');
          if (title) {
            // 기존 태그 텍스트 추출 (첫 번째 태그, Vv 제거)
            var origTag = card.querySelector('.gh-card-tag');
            var tagText = origTag ? (origTag.textContent || '').trim() : '';
            if (tagText) {
              // 종종 "Vvai" 같이 뜼으므로 stripVv 적용
              tagText = stripVv(tagText);
              // 이미 제목이 있는 h3 안에 span 으로 삽입 (title 자체가 flex row)
              var tagEl = document.createElement('span');
              tagEl.className = 'cl-list-tag';
              tagEl.textContent = tagText;
              title.insertBefore(tagEl, title.firstChild);
            }
          }
        }
