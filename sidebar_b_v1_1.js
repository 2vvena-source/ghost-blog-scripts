/*!
 * SIDEBAR-B v1.1 STEP 1 (external hosted)
 * 배포: 2026-08-21
 * 로더 위치: Ghost Site Footer (single <script src=...> tag)
 * 
 * v1.1 변경사항 (v1 대비):
 *   [+] 프로필 = 양쪽 페이지 최상단 공용 (슬라이드 영역 밖으로 이동)
 *   [+] 페이지 폭 넘침 수정 (트랙 width/box-sizing 정확 계산)
 *   [+] 인디케이터 = 화면 최하단 고정 (position: fixed, 사이드바 폭 안에서)
 *   [+] 썸네일 세로 이미지 대응 (max-height 520px)
 *   [+] publication-cover 는 "썸네일 없음" 으로 처리 (기본 커버 무시)
 * 
 * 다음 STEP:
...
(function(){
  ...
  var CSS_TEXT = '' +
    ...
    /* ---- 썸네일 없는 경우 (홈 카드 no-image 스타일과 통일) ---- */
    ...
    '    var(--color, #0F3A3A) 51%, transparent 51%) !important;' +
  ...
  function renderSidebarB(){
    try {
      ...
      if (featureImg) {
        ...
        titleBox.textContent = getPostTitle();
