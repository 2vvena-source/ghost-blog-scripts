...
(function(){
  ...
  var CSS = ''
    ...
    //   기본 background = 사선 빗금 (no-image 때 그대로 노출)
  ...
  function injectCSS(){
    try {
      ...
      style.setAttribute('data-ddl-cat-landing', 'v13');
  ...
  function markNoImageCards(){
    ...
    cards.forEach(function(card){
      ...
      if (!img || !img.getAttribute('src')) {
        // v13: no-image 클래스 부여 안 함. 오직 빈 .gh-card-image div 삽입해서
        //      with-image 와 DOM 구조를 완전히 같게 만듦.
        //      사선 빗금은 with-image .gh-card-image 의 기본 background 로 자동 적용.
        if (!imgBox) {
