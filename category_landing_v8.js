/*!
 * CATEGORY-LANDING v8 (external hosted)
 *
...
(function(){
  ...
  var CSS = ''
    ...
    + '  left: 0;'
    + '}'
    // 이미지 없는 카드: 좌측 썸네일 자리 = 사선 빗금 + 그 안에 매우 연한 손글씨체로 글 제목 (세로)
    //   3행 라벨 구조 자체는 with-image 와 동일 유지.
    ...
    + '}'
    // no-image 썸네일 자리 안의 제목 오버레이 (JS 로 삽입, cl-noimg-title)
    + '.cl-feed.cl-view-list .cl-noimg-title {'
  ...
  function injectCSS(){
    try {
      ...
      style.setAttribute('data-ddl-cat-landing', 'v8');
  ...
  function enrichCards(){
    try {
      ...
      cards.forEach(function(card, idx){
        ...
        // no-image 카드: 썸네일 자리에 글 제목을 연한 세로텍스트로 삽입
        if (card.classList.contains('no-image')) {
          var imgBox = card.querySelector('.gh-card-image');
          if (imgBox && !imgBox.querySelector('.cl-noimg-title')) {
            var titleEl = card.querySelector('.gh-card-title');
            var titleText = titleEl ? (titleEl.textContent || '').trim() : '';
            if (titleText) {
              var noImgTitle = document.createElement('div');
              noImgTitle.className = 'cl-noimg-title';
              noImgTitle.textContent = titleText;
              imgBox.appendChild(noImgTitle);
            }
          }
        }
      });
