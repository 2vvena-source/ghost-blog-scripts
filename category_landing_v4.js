/*!
 * CATEGORY-LANDING v4 (external hosted)
 * 배포: 2026-08-21
 * 로더 위치: Ghost Site Footer
 *
 * v4 변경 (v3 대비):
 *   [!] v3 는 커스텀 헤더를 <header> 태그로 만들어 .gh-main 의 직계 자식으로 삽입 →
 *       Ghost Source 테마 CSS 의 main > header 규칙(외부 CDN, CORS 로 검사 불가) 에 걸려
 *       display:none 처리되어 화면에 안 보였음.
 *   [!] <header> → <div> 로 태그만 변경. 클래스명(.cl-header) 은 유지. main > header 셀렉터
 *       에 매칭 안 되어 정상 노출.
 *
 * v3 배경 (남겨둠):
...
(function(){
  ...
  function injectCSS(){
    try {
      ...
      style.setAttribute('data-ddl-cat-landing', 'v4');
