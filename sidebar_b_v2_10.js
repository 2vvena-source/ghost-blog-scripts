/*!
 * SIDEBAR-B v2.10 (external hosted)
 * 배포: 2026-08-27
 * 로더 위치: Ghost Site Footer (single <script src=...> tag)
 *
 * v2.10 변경사항 (v2.9 대비):
 *   [!] 진입 조건 엄격화 — 개별 글 페이지에서만 대보임
 *       - 다음 중 하나라도 해당되면 진입 금지:
 ...
 *       - codeinjection_head 에 <!--HL-*--> 저장 방식은 마커 밖 내용이 head 에 해석되어
 *         페이지 상단으로 새어나오는 문제 발견 → 저장 방식 자체 변경 필요
 *       - TASK D (Tampermonkey 편집 UI) 에서 GitHub 저장소 방식으로 재구현 예정
 *   [+] 파일명 v2.9 상향
 * 
...
(function(){
  ...
  // ============================================================
  function isPostPage(){
    // v2.10: 먼저 확실한 '개별 글이 아닌' 신호가 있으면 즉시 거부
    var body = document.body;
    if (body && body.classList){
      var blockTemplates = ['tag-template','page-template','home-template','author-template','archive-template','index-template','paged-template'];
      for (var i = 0; i < blockTemplates.length; i++){
        if (body.classList.contains(blockTemplates[i])) return false;
      }
    }
    var p = (window.location && window.location.pathname) || '';
    var blockPrefixes = ['/tag/','/author/','/series/','/categories/','/tags/','/page/'];
    for (var j = 0; j < blockPrefixes.length; j++){
      if (p.indexOf(blockPrefixes[j]) === 0) return false;
    }
    // 그다음에 개별 글이라는 마커 검사
    if (body && body.classList.contains('post-template')) return true;
    // (article.gh-article 또는 section.gh-content) 임어도 둘 다 있을 때만 허용 (하나면 시리즈에서도 오개가능)
    var ha = !!document.querySelector('article.gh-article');
    var hs = !!document.querySelector('section.gh-content');
  ...
  function renderSidebarB(){
    try {
      ...
      // v2.9: 하이라이트 블록은 함수 자체 명백으로 비활성화 (TASK D 이월)
      //       renderHighlightBlock 은 남겨둔 코드이며 호출하지 않음
      // (v2.8 를 통째 삭제하지 않는 이유: TASK D 서 표시 로직 재사용 예정)
