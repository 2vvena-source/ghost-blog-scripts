/*!
 * CATEGORY-LANDING v12 (external hosted)
 *
 * v12 변경 (v11 대비):
 *   [!] v11 은 JS 로 빈 .gh-card-image div 를 삽입하긴 했으나,
 *       flex 생자 폭 계산 오류로 누락/폭 이상 발생.
 *   [!] v12: no-image 카드 전용 CSS 규칙 — flex/width/height/background
 *       모두 !important 로 명시적 명시 → 무조건 고정 110px 박스 + 빗금 배경
 *       + JS 삽입 박스에 data-cl-noimg="1" 마커를 붙여 CSS 적용 보장
 *
 *
...
(function(){
  ...
  var CSS = ''
    ...
    + '  left: 0;'
    + '}'
    // 이미지 없는 카드 — v12: data-cl-noimg="1" 마커 + 명시적 !important 로 무조건 고정
    + '.cl-feed.cl-view-list .gh-card-image[data-cl-noimg="1"] {'
    + '  width: 110px !important;'
    ...
    + '  background-image: linear-gradient(-45deg, transparent 46%, rgba(15,58,58,0.35) 46%, rgba(15,58,58,0.35) 54%, transparent 54%) !important;'
    ...
    + '  display: block !important;'
    + '}'
    + '.cl-feed.cl-view-list .gh-card-image[data-cl-noimg="1"] img {'
  ...
  function injectCSS(){
    try {
      ...
      style.setAttribute('data-ddl-cat-landing', 'v12');
      ...
      if (!img || !img.getAttribute('src')) {
        ...
        if (!imgBox) {
          ...
          if (link) {
            ...
            box.setAttribute('data-cl-noimg', '1');
            link.insertBefore(box, link.firstChild);
          }
        } else {
          // 이미 있는 imgBox 에도 마커 (배경 빗금 적용용)
          imgBox.setAttribute('data-cl-noimg', '1');
        }
      }
    });
  }
