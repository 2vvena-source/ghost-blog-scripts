/*!
 * SIDEBAR-B v2.3 STEP 2 (external hosted)
 * 배포: 2026-08-21
...
(function(){
  ...
  var CSS_TEXT = '' +
    ...
    '}' +
    /* v2.3: 시리즈 현재 회차 안내 문구 */
    '.sb-series-current-info {' +
    '  font-family: \'Pretendard Variable\',\'Pretendard\',sans-serif;' +
    '  font-size: 0.75em;' +
    '  color: rgba(15, 58, 58, 0.55);' +
  ...
  function renderSeriesBlock(blockEl){
    ...
    fetch(url)
      ...
      .then(function(data){
        ...
        // v2.3: 초기 상태에서 "현재 회차" 안내 문구 표시
        //   시리즈 목록이 아직 축소 상태일 때, 사용자가 "지금 몇 번째 회차인지" 명확히 알게
        var currentInfoLabel = document.createElement('div');
        currentInfoLabel.className = 'sb-series-current-info';
        function updateCurrentInfo(){
          if (currentShownCount >= posts.length) {
            currentInfoLabel.style.display = 'none';
            return;
          }
          if (currentIdx >= 0) {
            var totalMsg = '전 ' + posts.length + '회 중';
            var currentMsg = '현재 ' + String(currentIdx + 1).padStart(2, '0') + '화';
            var shownMsg = '표시 ' + currentShownCount + '개';
            currentInfoLabel.textContent = totalMsg + ' · ' + currentMsg + ' · ' + shownMsg;
          } else {
            currentInfoLabel.textContent = '전 ' + posts.length + '회 · 표시 ' + currentShownCount + '개';
          }
          currentInfoLabel.style.display = '';
        }
        body.insertBefore(currentInfoLabel, ul);
        updateCurrentInfo();
        ...
        // 접기 상태 복원 (제목 텍스트 클릭 = 시리즈 페이지 이동, 우측 아이콘 = 접기)
        applyCollapseState(blockEl, titleWrap.storageKey, { titleHref: seriesPageURL });
      })
      .catch(function(e){
  ...
  function renderTocBlock(blockEl){
    ...
    // 편집기 카드 내부 헤딩 제외 (토글카드, 헤더카드, 콜아웃 등)
    ...
    for (var hi = 0; hi < allHeadings.length; hi++){
      ...
      // 자기 자신이 제외 대상 클래스면 스킵
      if (hh.matches && hh.matches(EXCLUDE_HEADING_SELECTORS)) continue;
      // 조상 중에 카드 컨테이너가 있으면 스킵 (카드 내부 헤딩)
      var parent = hh.closest('.kg-toggle-card, .kg-header-card, .kg-callout-card, .kg-file-card, .kg-product-card, .kg-bookmark-card');
      if (parent) continue;
      headings.push(hh);
    }
    if (!headings.length) {
