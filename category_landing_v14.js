/*!
 * CATEGORY-LANDING v14 (external hosted)
 *
 * v14 변경 (v13 대비) — 리스트뷰 no-image 카드 정상화:
 *   [!] 카드 세로 폭주 차단: .cl-view-list .gh-card 에 max-height: 200px + overflow:hidden
 *       (v13은 요약이 길어지면 좌측 슬롯이 452px 까지 늘어남 → 시각 이상)
 *   [!] 요약 행 실제 1줄로 제한 (-webkit-line-clamp:1) — flex 안에서 white-space:nowrap 만으로는 부족
 *   [!] no-image 카드 좌측 슬롯 안에 손글씨 글제목 삽입:
 *       · 홈화면 빗금썸네일과 완전 동일 스펙: 빗금 4px 4px, 49~51%, var(--color)
 *       · 손글씨 폰트 NanumURiDdarSonGeurSsi + opacity 0.6 + 흰 스트로크
 *       · 세로 슬롯 110px 안에서 자연스레 잘림 (word-break:break-all, overflow:hidden)
 *       · JS: markNoImageCards 에서 빈 .gh-card-image 안에 <span class="cl-noimg-title-slot"> 삽입
 *   [!] 카드뷰의 v13 잔재 규칙 .gh-card.no-image .gh-card-image 삭제
 *       (with-image 규칙이 그대로 걸림 = v13 리스트뷰 원칙 그대로)
 *
 * 배포: 2026-08-21 세션 5
...
(function(){
  ...
  var CSS = ''
    ...
    + '}'
    // v14: 카드뷰 no-image 잔재 규칙 삭제. with-image 규칙이 그대로 걸림.
    ...
    + '  overflow: hidden;'
    + '  min-height: 160px;'
    + '  max-height: 200px !important;'   /* v14: 폭주 차단 (with/no-image 공통) */
    + '}'
    ...
    + '  max-height: 200px !important;'   /* v14: 링크 레벨도 동일 상한 */
    + '  text-decoration: none !important;'
    + '  color: inherit !important;'
    + '  position: relative;'
    + '  overflow: hidden !important;'    /* v14: 안전망 */
    + '}'
    // ▷ 좌측 세로 직사각 썸네일 박스 (with-image / no-image 동일 규칙)
    //   v14: 홈 빗금썸네일과 동일 스펙 (4px 4px, 49~51%, var(--color))
    //   이미지 있으면 <img> 가 그 위에 덮으므로 빗금 안 보임
    ...
    + '  min-height: 160px;'
    + '  max-height: 200px !important;'   /* v14: 슬롯도 동일 상한 */
    + '  align-self: stretch;'
    + '  position: relative;'
    + '  overflow: hidden;'
    + '  background-color: var(--base, #F5F5F5) !important;'
    + '  background-image: linear-gradient(-45deg, transparent 49%, var(--color) 49%, var(--color) 51%, transparent 51%) !important;'
    + '  background-size: 4px 4px !important;'
    + '  background-repeat: repeat !important;'
    + '  border-right: 1px solid var(--cl-line);'
    + '}'
    // v14: no-image 슬롯 안 손글씨 글제목 (홈 .gh-card.no-image .gh-card-excerpt 이식)
    + '.cl-feed.cl-view-list .gh-card-image .cl-noimg-title-slot {'
    + '  display: block;'
    + '  width: 100%;'
    + '  height: 100%;'
    + '  box-sizing: border-box;'
    + '  padding: 0.6em 0.5em;'
    + '  font-family: "NanumURiDdarSonGeurSsi","Nanum Pen Script",cursive !important;'
    + '  font-size: 1.2em;'
    + '  font-weight: 300;'
    + '  line-height: 1.15;'
    + '  letter-spacing: 0.01em;'
    ...
    + '  position: relative;'
    + '  text-shadow:'
    + '    -1px -1px 0 var(--base, #F5F5F5), 0 -1px 0 var(--base, #F5F5F5), 1px -1px 0 var(--base, #F5F5F5),'
    + '    -1px  0   0 var(--base, #F5F5F5),                                 1px  0   0 var(--base, #F5F5F5),'
    + '    -1px  1px 0 var(--base, #F5F5F5), 0  1px 0 var(--base, #F5F5F5), 1px  1px 0 var(--base, #F5F5F5);'
    + '}'
    ...
    + '  left: 0;'
    + '}'
    // v13: no-image 전용 CSS 규칙 전부 삭제 — with-image 규칙이 그대로 걸림
    // ▷ 우측 정보영역 — 3행 구조
    ...
    + '  min-width: 0;'
    + '  max-height: 3.2em !important;'   /* v14: 물리적 상한 */
    + '  white-space: nowrap !important;'
    + '  text-overflow: ellipsis;'
    + '  font-family: "Pretendard Variable","Pretendard",sans-serif !important;'
    + '}'
    // v14: 요약 실제 텍스트 노드도 잘리게 (flex 자식 안 넘침)
    + '.cl-feed.cl-view-list .gh-card-excerpt {'
  ...
  function injectCSS(){
    try {
      ...
      style.setAttribute('data-ddl-cat-landing', 'v14');
  ...
  function markNoImageCards(){
    ...
    cards.forEach(function(card){
      ...
      if (!img || !img.getAttribute('src')) {
        // v14: with-image 와 DOM 구조 통일 + 슬롯 안에 손글씨 글제목 삽입
        //      (홈 .gh-card.no-image .gh-card-excerpt 감성 이식, A안)
        if (!imgBox) {
          ...
          if (link) {
            ...
            if (titleText) {
              ...
              slot.className = 'cl-noimg-title-slot';
              slot.textContent = titleText;
              box.appendChild(slot);
            }
            link.insertBefore(box, link.firstChild);
          }
        } else if (!imgBox.querySelector('.cl-noimg-title-slot')) {
          // 이미 빈 imgBox 가 있는데 slot 이 없는 경우 (재실행 안전망)
          var titleEl2 = card.querySelector('.gh-card-title');
          var titleText2 = titleEl2 ? (titleEl2.textContent || '').trim() : '';
          if (titleText2) {
            var slot2 = document.createElement('span');
            slot2.className = 'cl-noimg-title-slot';
            slot2.textContent = titleText2;
            imgBox.appendChild(slot2);
          }
        }
      }
    });
  ...
  // v13: 넘버링/알약태그/세로제목 오버레이 모두 삭제. no-excerpt 마커만 부여.
