/*!
 * MEMBER-FILTER v1 (external hosted)
 * 배포: 2026-08-21
 * 로더 위치: Ghost Site Footer (single <script src=...> tag)
 * 
 * 역할:
 *   1. 비로그인 방문자에게 멤버공개글(자물쇠) 카드 완전 제거
 *   2. 비로그인 방문자에게 Hidden-* 태그 붙은 글 카드 완전 제거
 *      (hash-hidden-, #hidden- 등 대소문자 무관)
 *   3. RSS / sitemap 링크 페이지 노출 억제
 *   4. Portal 검색 결과 최대 은닉 시도
 * 
 * 안전장치:
 *   - 관리자 페이지(/ghost/*)에서는 즉시 종료 (EDITOR-SAFE 원칙)
 *   - window.IS_MEMBER === true 이면 즉시 종료 (관리자 본인은 다 보임)
 *   - 중복 로드 방지 (window.__DDL_MEMBER_FILTER_LOADED)
 */
(function(){
  'use strict';
...
(function(){
  ...
  if (window.IS_MEMBER === true) {
    // 관리자 본인. 필터 미적용.
    // 다만 관리자에게 "숨김 상태임" 표시를 위해 body 클래스만 부여.
